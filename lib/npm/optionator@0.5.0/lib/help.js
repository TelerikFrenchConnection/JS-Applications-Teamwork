...
(function() {
  ...
  getPreText = function(option, arg$, maxWidth) {
    ...
    if (maxWidth != null && !option.boolean && initialIndent + namesStringLen + typeSeparatorStringLen + typeString.length > maxWidth) {
      ...
      return namesString + "" + (option.boolean ? '' : typeSeparatorString + "" + typeString);
    }
  };
      ...
      for (i$ = 0, len$ = (ref$ = options).length; i$ < len$; ++i$) {
        ...
        if (showHidden || !item.hidden) {
          if (that = item.heading) {
            ...
            desc = item['default'] && !item.negateName ? (that = item.description) != null ? that + " - default: " + item['default'] : "default: " + item['default'] : (that = item.description) != null ? that : '';
            data.push({
              type: 'option',
              pre: pre,
              desc: desc,
