...
(function(process) {
  ...
  core.languageProcessors = {javascript: require("./languages/javascript").javascript};
  ...
  var nonInheritedTags = new Set(['article', 'section', 'nav', 'aside', 'hgroup', 'header', 'footer', 'address', 'dt', 'dd', 'figure', 'figcaption', 'main', 'em', 'strong', 'small', 's', 'cite', 'dfn', 'abbr', 'ruby', 'rt', 'rp', 'code', 'var', 'samp', 'kbd', 'i', 'b', 'u', 'mark', 'bdi', 'bdo', 'wbr']);
  ...
  define('HTMLElement', {
    ...
    proto: {
      dispatchEvent: function(event) {
        ...
        if (!event._canceled && event.target._eventDefaults[event.type] && typeof event.target._eventDefaults[event.type] === 'function') {
      ...
      click: function() {
        ...
        var event = new core.MouseEvent("click", {
          bubbles: true,
          cancelable: true
        });
    ...
    proto: {
      ...
      get elements() {
        return new core.HTMLCollection(this._ownerDocument, mapper(this, function(e) {
          return listedElements.test(e.nodeName);
        }));
      },
      get length() {
      ...
      _askedForAReset: function() {
        ...
        if ((!this.size || this.size === 1) && !selected.length) {
          for (var i = 0; i < options.length; ++i) {
            ...
            if (parentNode && parentNode.nodeName.toUpperCase() === 'OPTGROUP' && parentNode.disabled) {
              disabled = true;
            }
            if (!disabled) {
  ...
  define('HTMLInputElement', {
    ...
    proto: {
      ...
      select: function() {},
      _eventDefaults: {click: function(event) {
  ...
  define('HTMLAnchorElement', {
    ...
    proto: {_attrModified: function(name, value, oldVal) {
        if (name === 'href') {
          whatwgUrl.setTheInput(this, value);
        }
        core.HTMLElement.prototype._attrModified.call(this, name, value, oldVal);
      }},
    attributes: ['accessKey', 'charset', 'coords', 'hreflang', 'name', 'rel', 'rev', 'shape', {
      prop: 'tabIndex',
      type: 'long'
    }, 'target', 'type']
  });
  ...
  define('HTMLMediaElement', {
    ...
    attributes: [{
      ...
      prop: 'controls',
      type: 'boolean'
    }, 'crossOrigin', 'currentTime', 'preload', {
      prop: 'loop',
      type: 'boolean'
    }, 'mediaGroup']
  });
