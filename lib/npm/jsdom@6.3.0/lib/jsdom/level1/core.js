...
(function(process) {
  ...
  core.NodeList = function NodeList(element, query) {
    if (!query) {
      ...
      Object.defineProperties(this, {_length: {
  ...
  core.DOMImplementation = function DOMImplementation(document, features) {
  ...
  core.DOMImplementation.prototype = {
    ...
    _hasFeature: function(feature, version) {
    ...
    _attrModified: function(name, value, oldValue) {
      ...
      if (name.length > 2 && name[0] === 'o' && name[1] === 'n') {
        if (value) {
          ...
          var vmOptions = {
            filename: this._ownerDocument._URL,
            displayErrors: false
          };
      ...
      if (value !== oldValue && this._ownerDocument && this._ownerDocument.implementation._hasFeature('MutationEvents')) {
  ...
  inheritFrom(core.Node, core.Element, {
    ...
    set outerHTML(html) {
      ...
      if (parent.nodeType === NODE_TYPE.DOCUMENT_NODE) {
        throw new core.DOMException(core.DOMException.NO_MODIFICATION_ALLOWED_ERR, "Modifications are not allowed for this document");
    ...
    scrollTop: 0,
    scrollLeft: 0,
    getElementsByTagName: memoizeQuery(function(name) {
  ...
  inheritFrom(core.Node, core.DocumentFragment, {nodeType: NODE_TYPE.DOCUMENT_FRAGMENT_NODE});
  core.Document = function Document(options) {
    ...
    this._activeNodeIteratorsMax = options.concurrentNodeIterators === undefined ? 10 : Number(options.concurrentNodeIterators);
    ...
    getElementsByTagName: memoizeQuery(function(name) {
      function filterByTagName(child) {
        if (child.nodeName && child.nodeType === NODE_TYPE.ELEMENT_NODE) {
          if (name === "*") {
            return true;
          } else if (child._ownerDocument && child._ownerDocument._doctype && child.nodeName.toLowerCase() === name.toLowerCase()) {
            return true;
  ...
  inheritFrom(core.Node, core.Attr, {
    ...
    insertBefore: function(newChild, refChild) {
      if (newChild.nodeType === NODE_TYPE.CDATA_SECTION_NODE || newChild.nodeType === NODE_TYPE.ELEMENT_NODE) {
        throw new core.DOMException(core.DOMException.HIERARCHY_REQUEST_ERR);
      }
      return core.Node.prototype.insertBefore.call(this, newChild, refChild);
    },
    appendChild: function(arg) {
      if (arg.nodeType === NODE_TYPE.CDATA_SECTION_NODE || arg.nodeType === NODE_TYPE.ELEMENT_NODE) {
        throw new core.DOMException(core.DOMException.HIERARCHY_REQUEST_ERR);
      }
      return core.Node.prototype.appendChild.call(this, arg);
    }
